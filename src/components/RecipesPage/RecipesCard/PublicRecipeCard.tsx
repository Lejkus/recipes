...
export default function PublicRecipeCard({ recipe, index, currentUser,deleteFromSaved,addToSaved }) {
    console.log('render');
    ...
    return <div className='recipe-card' key={index} >
        <div className='header' style={recipe.images[0] ? { backgroundImage: `url(${recipe.images[0]})` } : { backgroundImage: `url(https://boodabike.com/wp-content/uploads/2023/03/no-image.jpg)` }}  >
            {recipe.userId !== currentUser ? recipe.usersShared && recipe.usersShared.includes(currentUser) ? <button className='edit' onClick={() => { deleteFromSaved(recipe) }}><FontAwesomeIcon icon={faEraser} /></button> : <button className='edit' onClick={() => { addToSaved(recipe) }}><FontAwesomeIcon icon={faDownload} /></button> : <></>}
        </div>
